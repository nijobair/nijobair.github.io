...
In Vega, datasets are defined inside the `data` block. As mentioned in the [previous post](/posts/powerbi/004-core-syntax), this block takes an array, which means you can define multiple datasets—each one as a JSON object.
...
![An empty dataset in Vega](/assets/images/posts/powerbi/005-01-empty-dataset.png){: .has-caption.post-img.has-border.ml-auto.mr-auto}
{: data-caption="Figure: An empty dataset in Vega"}
...
![Dataset in Deneb](/assets/images/posts/powerbi/005-02-dataset-in-deneb.png){: .has-caption.post-img.has-border.ml-auto.mr-auto}
{: data-caption="Figure: Dataset in Deneb"}
...
Vega provides a collection of sample datasets in various formats that are perfect for learning and experimenting. You can explore them [here](https://cdn.jsdelivr.net/npm/vega-datasets@3.1.0/data/){:target="_blank" rel="noopener"}. To use any of these datasets, just set the `url` like this: `"https://cdn.jsdelivr.net/npm/vega-datasets@3.1.0/data/<datasetName.format>"`. Replace `<datasetName.format>` with the actual name and format of the dataset you want to use.
